...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($input_pass)) {
      ...
      if(preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})/", $input_pass)){
        $pass = $input_pass;
      }
      else{
        echo "<script> alert('Your password must be between 8 and 30 characters. Your password must contain at least one uppercase letter, lowercase letter, at least one digit, and at least one special character');";
